...
RUN_NAME = "MYC_newComparison_recapRun1WeakerB3_blockingRegion-0.75cap_0.03rel" #### Define a name for this simulation run, sowe can save parameters
...
TIER3_BLOCK = [0.5, 0.05] ### Increasing probability of release
TIERB3_BLOCK = [0.4, 0.02]
TIER4_BLOCK = [0.25, 0.03]

### Defining 'blocking regions' - consecutive monomers that can block
E1_blockingRegion = np.arange(180, 221)
E2_blockingRegion = np.arange(311, 320)
blockingRegions = [E1_blockingRegion, E2_blockingRegion]
for br in blockingRegions:
    for loc in br:
        left_blockers_capture[loc] = 0.75
        left_blockers_release[loc] = 0.03
        right_blockers_capture[loc] = 0.75
        right_blockers_release[loc] = 0.03
...
E2 = 315
B3 = 555
#TARGETED_LOADING_SPOTS = [E1_2+1,E2+1,B3+1]
TARGETED_LOADING_SPOTS = []
