...
class Extruder_cy():
    def __init__(self, extruder_index, left_blockers_capture, right_blockers_capture, left_blockers_release, right_blockers_release, extrusion_occupancy, leg1=None, leg2=None, lifetime=100, lifetime_stalled=10, targeted_loading=False, loading_spots=None, loading_dist=None):
        """
        ...
        Parameters:
            ...
            left/right_blockers - dicts: each dict of form {pos:prob}, where pos is position of blocker and prob is capture probability [0,1]. Left-facing blockers and right-facing in respective dicts
            ...
            lifetime - defines the unload probability via (1/lifetime)
            lifetime_stalled - defines the stalled unload probability via (1/lifetime_stalled)
            leg1/2 - int, spot on the polymer to load the extruder. In targeted loading, these are None as the loading sites are predifned in loading_spots
        """
    ...
    def loadNew(self):
        ...
        while True:
            if self.targeted_loading: # This is where the ""magic"" happens for targeted loading!
                ...
                # Since we have the loading distributions in a CDF format (cumulative probabilities), we just need to find the first index greater than p to know where to load the targeted extruder.
                for i,x in enumerate(self.loading_dist): 
                    if p > x: 
                        continue
                    elif p <= x:
                        pos = self.loading_spots[i] # We have found the position to use
                        break
                pos = self.loading_spots[i] #I don't think this is needed if we use p <= x above...
            else: # If not targeted loading, pick a random spot
                pos = np.random.randint(len(self.occupied)-1)
            if self.occupied[pos] != 1 and self.occupied[pos+1] != 1: # Check to see if the attempted loading spots are already occupied. If they are, the while loop repeats.
                break
        self.leg1.pos = pos
        self.leg2.pos = pos+1
...
# We should never call this class from the command line, only import it
